...
# 131.39: pinned private copy (cmd_run pins once, workers reuse via $TOKE_TKC_PIN)
...
def cmd_run(args):
    ...
    # 131.39: pin the compiler before the pool starts; every row carries its sha
    pinned = tkc_pin.pin().install(sys.modules[__name__], validate, metrics, idiom_judge)
    print(f"auditing {len(jobs)} records ({len(done) - len(jobs)} already done), "
          f"{args.workers} workers, tkc {pinned.version} sha256 {pinned.sha256[:12]}", file=sys.stderr)
    n = ok_new = 0
    with pinned, multiprocessing.Pool(args.workers) as pool, open(ledger_path, "a") as led:
        for row in pool.imap_unordered(audit_one, jobs, chunksize=8):
            row["tkc_bin_sha"] = pinned.sha256
    ...
    print(json.dumps({"audited": n, "ledger": ledger_path, "tkc_bin_sha": pinned.sha256}))
...
def cmd_report(args):
    ...
    summary = {"total": len(rows), "buckets": dict(buckets),
               # 131.39: every binary the rows were judged on (one entry = one clean sweep)
               "tkc_bin_sha": sorted({r["tkc_bin_sha"] for r in rows if r.get("tkc_bin_sha")}),
